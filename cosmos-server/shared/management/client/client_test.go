...
import (
	...
	ephemeral_manager "github.com/ethancls/cosmos-server/internal/modules/peers/ephemeral/manager"
...
func startManagement(t *testing.T) (*grpc.Server, net.Listener) {
	...
	networkMapController := controller.NewController(ctx, store, metrics, updateManager, requestBuffer, mgmt.MockIntegratedValidator{}, settingsMockManager, "cosmos.selfhosted", port_forwarding.NewControllerMock(), ephemeral_manager.NewEphemeralManager(store, peersManger), config)
