...
const (
	fileGeneratedResolvConfContentHeader         = "# Generated by Cosmos"
...
# The original file can be restored from ` + fileDefaultResolvConfBackupLocation + "\n\n"
	...
	fileDefaultResolvConfBackupLocation = defaultResolvConfPath + ".original.cosmos"
...
func (f *fileConfigurator) updateConfig(nbSearchDomains []string, nbNameserverIP netip.Addr, cfg *resolvConf, stateManager *statemanager.Manager) error {
	...
	log.Infof("created a Cosmos managed %s file with the DNS settings. Added %d search domains. Search list: %s", defaultResolvConfPath, len(searchDomainList), searchDomainList)
...
func (f *fileConfigurator) restoreUncleanShutdownDNS(storedDNSAddress netip.Addr) error {
	...
	// current address is still cosmos's non-available dns address -> restore
	currentDNSAddress := resolvConf.nameServers[0]
	if currentDNSAddress == storedDNSAddress {
		return restoreResolvConfFile()
	}

	log.Infof("restoring unclean shutdown: first current nameserver differs from saved nameserver pre-cosmos: %s (current) vs %s (stored): not restoring", currentDNSAddress, storedDNSAddress)
	return nil
}
