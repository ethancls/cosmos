...
func init() {
	defaultConfigPathDir = "/etc/cosmos/"
	defaultLogFileDir = "/var/log/cosmos/"
	...
	case "freebsd":
		defaultConfigPathDir = "/var/db/cosmos/"
	}
	...
	defaultDaemonAddr := "unix:///var/run/cosmos.sock"
	...
	rootCmd.PersistentFlags().StringVarP(&logLevel, "log-level", "l", "info", "sets Cosmos log level")
	rootCmd.PersistentFlags().StringSliceVar(&logFiles, "log-file", []string{defaultLogFile}, "sets Cosmos log paths written to simultaneously. If `console` is specified the log will be output to stdout. If `syslog` is specified the log will be sent to syslog daemon. You can pass the flag multiple times or separate entries by `,` character")
	...
	rootCmd.PersistentFlags().BoolVarP(&anonymizeFlag, "anonymize", "A", false, "anonymize IP addresses and non-cosmos.io domains in logs and status output")
	...
	upCmd.PersistentFlags().StringVar(&customDNSAddress, dnsResolverAddress, "",
		`Sets a custom address for Cosmos's local DNS resolver. `+
