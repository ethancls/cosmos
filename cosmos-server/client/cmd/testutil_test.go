...
func startManagement(t *testing.T, config *config.Config, testFile string) (*grpc.Server, net.Listener) {
	...
	networkMapController := controller.NewController(ctx, store, metrics, updateManager, requestBuffer, mgmt.MockIntegratedValidator{}, settingsMockManager, "cosmos.cloud", port_forwarding.NewControllerMock(), manager.NewEphemeralManager(store, peersmanager), config)
