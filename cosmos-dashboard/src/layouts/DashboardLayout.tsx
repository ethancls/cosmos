...
function DashboardPageContent({
  ...
  return (
    <div className={cn("flex flex-col h-screen", mobileNavOpen && "flex")}>
      {mobileNavOpen && (
        <motion.div
          className={"h-screen bg-white dark:bg-nb-gray-950 w-11/12 max-w-[22rem]"}
          layout={true}
          transition={{
            type: "spring",
            stiffness: 100,
          ...
          >
            <div className={"flex items-center gap-3 max-w-[22rem]"}>
              ...
              <div className="flex flex-col space-y-1">
                <p className="font-medium leading-none text-gray-700 dark:text-gray-300">
                  {user?.name}
                </p>
      ...
      <AnimatePresence mode={"wait"}>
        <motion.div
          ...
          className={cn(
            mobileNavOpen
              ? "border border-gray-200 dark:border-nb-gray-900 shadow-inner overflow-hidden rounded-xl fixed scale-75"
              : "",
          )}
          transition={{
            type: "spring",
